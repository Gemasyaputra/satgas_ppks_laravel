...
<div class="container-fluid">
    ...
    <div class="d-flex flex-wrap align-items-center gap-2 mb-4 border-bottom pb-3">
        ...
        @endphp
        <span class="badge {{ $statusBadge[$report->status] ?? 'bg-secondary' }} px-3 py-2 rounded-pill">
            {{ $statusText[$report->status] ?? $report->status }}
        </span>
        <span class="badge bg-info-subtle text-info-emphasis px-3 py-2 rounded-pill">{{ $report->category }}</span>
        ...
        <small class="text-muted ms-auto">
            <i class="bi bi-calendar-event me-1"></i> Dibuat: {{ $report->created_at->format('d M Y') }}
        </small>
    </div>
    ...
    <div class="row g-4 mb-4">
            ...
            <div class="card bg-light border-0 h-100">
                <div class="card-body">
                    ...
                    <div class="mb-2">
                        ...
                        <span class="fw-medium text-dark">{{ \Carbon\Carbon::parse($report->report_date)->format('d F Y') }}</span>
                    </div>
        ...
        <div class="col-md-6">
            <div class="card bg-light border-0 h-100">
                <div class="card-body">
                    <h6 class="fw-bold text-danger mb-3"><i class="bi bi-person-exclamation me-2"></i>Terlapor & Saksi</h6>
                    ...
                    <div class="row">
                        <div class="col-6 mb-2">
                            ...
                            <span class="fw-medium text-dark">{{ $report->reported_party_occupation }}</span>
                        </div>
                        <div class="col-6 mb-2">
                            <small class="text-muted d-block">Perkiraan Usia</small>
                            <span class="fw-medium text-dark">{{ $report->reported_party_age }} Tahun</span>
                        </div>
                    </div>
    ...
    <div class="card border-primary-subtle mb-4">
        <div class="card-body bg-primary-subtle bg-opacity-10">
            <h6 class="fw-bold text-primary mb-3"><i class="bi bi-person-vcard-fill me-2"></i>Identitas Pelapor (Anda)</h6>
            <div class="row">
                ...
                <div class="col-md-4 mb-2">
                    ...
                    <span class="fw-medium">{{ $report->reporter_prodi }} / {{ $report->reporter_nim }}</span>
                </div>
                <div class="col-md-4 mb-2">
    ...
    <div class="mb-4">
        <h6 class="text-muted small text-uppercase fw-bold border-bottom pb-1 mb-2">Kronologi Lengkap</h6>
    ...
    <div class="row mb-4">
        <div class="col-md-6">
            <h6 class="text-muted small text-uppercase fw-bold border-bottom pb-1 mb-2">Alasan Melapor</h6>
            <p class="text-dark bg-light p-2 rounded small">{{ $report->reason_for_reporting }}</p>
        </div>
        <div class="col-md-6">
            <h6 class="text-muted small text-uppercase fw-bold border-bottom pb-1 mb-2">Kebutuhan Korban</h6>
    ...
    <div class="alert alert-secondary border-0 d-flex gap-3" role="alert">
        ...
        <div>
            ...
            <p class="mb-0" style="white-space: pre-wrap;">{{ $report->admin_notes ?? 'Belum ada catatan atau update terbaru dari admin mengenai laporan ini.' }}</p>
        </div>
    </div>
